...
public class DimensionArray {
	...
	public void testDemensionArray() {
		...
		//주소 배열의 크기가 3
		//데이터를 지정할 int[] 크기는 4
		//quize! 이때 heap영역에 생성되는 배열의 개수 ?
		//정담 : 4 -> 배열을 저장할 주소 배열 1개, int[] 3개
		...
		System.out.println("주소배열의 0번째 인덱스에 담겨있는 주소 값 : " + doubleArr[0]);
		System.out.println("주소배열의 0번째 인덱스로 참조되는 배열의 0번 인덱스의 값 : " + doubleArr[0][0]);
		System.out.println("Arrays.toString(doubleArr) : " + Arrays.toString(doubleArr));
		
		//반복문을 활용한 2차원 배열의 출력
		/*
		//주소배열의 길이 만큼 바깥쪽 반복문 반복
		for(int i = 0; i < doubleArr.length; i++) {
			System.out.println("주소배열의 " + i + "번 인덱스에 담겨이있는 배열");
		...
		*/
		//doubleArr2를 주소배열의 크기 10, in[]의 크기 10으로 메모리를 할당하고
		doubleArr2 = new int[10][10];
		//doubleArr2를 1~100까지의 수로 초기화 하세요.
		int inum = 1;
		for(int i = 0; i < doubleArr2.length; i++) {
			for(int j = 0; j < doubleArr2.length; j++) {
				doubleArr2[i][j] = inum;
				inum++;
			}
		}
		
		//doubleArr2 2차원 배열안에 들어있는 모든 값을 출력해주세요.
		for(int i = 0; i < doubleArr2.length; i++) {
			System.out.println("배열의 " + i + "번째 인덱스");
