...
public class Constructor {
	...
	//	클래스 변수 : jvm의 기본값 -> 필드변수에 직접 대입한 값 -> static 초기화 블록
	
	//필드변수
	//	인스턴스변수 : 클래스를 인스턴스화 했을 때 메모리에 올라가는 변수
	//	클래스변수 : 프로그램이 실행되면 메모리에 올라가는 변수
	private String memberStr = "하명도";	//인스턴스 변수
	private static int classInt = 10;	//클래스 변수
	...
	//	만약 다른 생성자가 없는데, 기본 생성자를 작성하지 않는다면 JVM이 자동으로 생성한다.
	//	생성자는 반환형이 없고, 이름은 클래스명과 같다.
	public Constructor() {
		super();	//부모클래스의 생성자를 호출
					//작성하지 않을 경우 자동으로 생성된다.
		
		System.out.println("Constructor클래스의 기본 생성자입니다.");
		System.out.println("생성자가 호출 되기 전 memberStr에 담겨 있는 값 : "
				+ memberStr);
		memberStr = "pclass";
	}
	public Constructor(String name) {
		System.out.println("Constructor클래스의 매개변수가 있는 생성자입니다.");
		System.out.println("생성자가 호출 되기 전 memberStr에 담겨 있는 값 : "
				+ memberStr);
		memberStr = name;
	}
