...
//			coffee.setAccount(account);
			...
			int no = i+1;
			System.out.print(no + "번째 커피의 등급을 등록하세요. 프리미엄:p | 일반:n : ");
			String type = sc.next();
			...
			case "p": coffee = new PremiumCoffee();
				coffee.setAccount(account);
				System.out.println("포장비용을 입력하세요 : ");
				//setPackageCost() 는 PremiumCoffee 클래스에 작성되어 있다.
				((PremiumCoffee)coffee).setPackageCost(sc.nextInt());
				break;
			case "n": coffee = new Coffee();
				coffee.setAccount(account);
				break;
			default: System.out.println("커피등급을 잘못 입력하였습니다.");
				i--; continue;
			}
