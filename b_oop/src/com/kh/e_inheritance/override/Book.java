...
public class Book {
	...
	public int hashCode() {
		...
		//해시함수 : 해싱을 위한 연산 처리를 해주는 함수들.
		// 1. 같은 값을 넣으면 언제나 같은 해싱값이 나와야 한다.
		// 2. 다른 값을 넣으면 언제나 다른 값이 나와야 한다. --> 이런 완벽한 해시알고리즘은 아직 없다.
		//	-> 다른 값을 넣었을 때 같은 해싱값이 반환되는 것을 해시충돌이라고 한다.
		
		//31이라는 숫자는 해시함수에서 관습적으로 많이 사용하는 값
		//31은 소수이면서 홀수인 수 -> 해시충돌을 최소화 할 수 있는 수.
		final int prime = 31;
		int result = 1;
		
		//author.hashCode : -> String.hashCode
		//String 클래스의 해시코드는 문자열 값의 해싱값을 반환하도록 오버라이드 되어있다.
	...
	public boolean equals(Object obj) {
		...
		if (autor == null) {
			if (other.autor != null)
				...
				//두 인스턴스의 author 속성이 다름이로 false로 반환
				return false;
		//String 타입인 author들을 비교하기 위해 equals()로 두 값을 비교
		} else if (!autor.equals(other.autor))
			return false; //author의 갑이 다르면 false로 반환
