...
public class A_FileTest {
	...
	//1. IO
	//입출력 : 외부자원(키보드, 파일, DB서버, API서버 등등)으로 부터
	...
	//만약 사용이 끝난 Stream을 닫아주지 않으면 JVM이 알아서 닫아준다. 하지만 예상하지 못한 예외상황이
	//발생할 수 있음으로 반드시 수동으로 닫아줘야 한다.
	
	//3. Stream의 구분
	//- 바이트스트림과 문자스트림으로 구분
	//바이트스트림은 Stream을 통해서 입출력되는 데이터의 크기가 1바이트 단위
	//문자스트림은 Stream을 통해 입출력되는 데이터의 크기가 2바이트 단위
	//-> 자바의 문자는 2바이트이기 때문에 바이트스트림을 통해 문자를 주고 받을 경우, 데이터가 유실되거나
	//	문제가 발생하는 경우가 있다. 2바이트 단위로 데이터를 입출력하는 문자스트림이 문자를 다룰때는 안전하다.
	
	//- 기반스트림과 보조스트림으로 구분
	//기반스트림 : 실제로 외부자원과 데이터를 입출력하는 스트림
	// FileInputStream/FileOutputStream				-파일 입출력 스트림
	// PipedInputStream/PipedOutputStream			-쓰레드간 데이터를 입출력
	// ByteArrayInputStream/ByteArrayOutputStream	-바이트 배열을 이용한 입출력
	...
	public void doFile() {
		...
		//경로 지정
		//절대경로 : root(windows에서는 C드라이브)부터 경로를 지정하는 방식, \\로 경로 구분
		...
		//		"test/pclass/easy" -> easy 폴더만 생성, 만약 pclass폴더가 없으면 에러
		...
		File dirs = new File("c:\\pclass\\io");
		dirs.mkdirs();
		...
		//파일 삭제
		dirsFile.delete();
		System.out.println("dirsFile에 지정된 경로의 파일이 존재합니까? " + dirsFile.exists());
